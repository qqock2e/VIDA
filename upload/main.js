...
const UI_STRINGS = {
  en: {
    ...
    btn_item:'🎴 Item',
    btn_bet_minus:'−',
    phase_betting:(round,revealed,item)=>
      `Round ${round} — ${revealed}/5 revealed · Bet: ♥`,
    phase_processing:'Processing…',
    result_profit:'🎉 Profit!', result_loss:'📉 Loss', result_fold:'🏳 Fold',
    result_life_now:(l)=>`Life: ${l}♥`,
    result_hand:'Hand', result_base_mult:'Base mult',
    result_suit_bon:(s)=>`Suit bonus (${s})`,
    result_hc_bon:'High-card bonus', result_alchemy:'Alchemist bonus',
    result_hh_bon:'High roller bonus',
    result_final_m:'Final mult', result_gain:'Earned',
    result_drain:'Drain', result_pts:'Points earned',
    result_fold_desc:'Only the round drain is applied.',
  ...
  ko: {
    ...
    phase_betting:(round,revealed)=>
      `라운드 ${round} — 필드 ${revealed}/5 공개 · 베팅:`,
    phase_processing:'처리 중…',
    result_profit:'🎉 수익!', result_loss:'📉 손실', result_fold:'🏳 폴드',
    result_life_now:(l)=>`라이프: ${l}♥`,
    result_hand:'족보', result_base_mult:'기본 배율',
    result_suit_bon:(s)=>`문양 보너스(${s})`,
    result_hc_bon:'하이카드 보너스', result_alchemy:'연금술사 보너스',
    result_hh_bon:'하이롤러 보너스',
    result_final_m:'최종 배율', result_gain:'획득',
    result_drain:'차감', result_pts:'포인트',
    result_fold_desc:'라운드 차감만 적용됩니다.',
...
// ─────────────────────────────────────────────
// AUDIO  (Web Audio API — tiny synth, no files)
// ─────────────────────────────────────────────
let audioCtx = null;
function getAudio() {
...
function revealNext() {
  ...
  sfx('reveal');
  const newCard=G.fieldCards[G.revealedCount-1];
...
function runHandAnimation(preview, onDone) {
  ...
  // Figure out which elements correspond to which cards
  const allCardEls = [
    ...document.querySelectorAll('#handCards .card'),
    ...document.querySelectorAll('#fieldCards .card'),
  ];

  // Map all[] index → DOM element
  ...
  const isBurst      = ['royalFlush','fourOfAKind','flush','fullHouse'].includes(preview.key);
...
function showResultModal(result) {
  const isBet=result.type==='bet';
  const isGain=result.netGain>=0;
  const HAND_MULTS=VidaGame.HAND_MULTS;
  const SUIT_SYMBOLS=VidaGame.SUIT_SYMBOLS;

  document.getElementById('resultTitle').textContent    = t(isBet?(isGain?'result_profit':'result_loss'):'result_fold');
  document.getElementById('resultDelta').textContent    = (isGain?'+':'')+result.netGain.toFixed(2)+' ♥';
  document.getElementById('resultDelta').className      = 'result-life-delta'+(isGain?'':' loss');
  document.getElementById('resultSubtitle').textContent = t('result_life_now',result.lifeAfter.toFixed(1));

  let bd='';
  if (isBet) {
    const sym=result.suitBonus?.suit?SUIT_SYMBOLS[result.suitBonus.suit]:'';
    bd=`${t('result_hand')}: <b>${VidaGame.getHandName(result.handKey,lang)}</b><br>`
      +`${t('result_base_mult')}: ×${(HAND_MULTS[result.handKey]||0).toFixed(2)}<br>`
      +(result.suitBonus?.bonus?`${t('result_suit_bon',sym)}: +${(result.suitBonus.bonus*100).toFixed(1)}%<br>`:'')
      +(result.hcBonus?`${t('result_hc_bon')}: +${result.hcBonus.toFixed(3)}<br>`:'')
      +(result.multBonus?`${t('result_alchemy')}: +${(result.multBonus*100).toFixed(0)}%<br>`:'')
      +(result.hhBonus?`${t('result_hh_bon')}: +${(result.hhBonus*100).toFixed(0)}%<br>`:'')
      +`<b>${t('result_final_m')}: ×${result.mult.toFixed(3)}</b><br>`
      +`${t('result_gain')}: +${result.lifeGain.toFixed(2)}♥ &nbsp;|&nbsp; ${t('result_drain')}: −${result.lifeCost.toFixed(2)}♥<br>`
      +`${t('result_pts')}: +${result.pointsEarned}P`;
  } else bd=t('result_fold_desc');

  document.getElementById('resultBreakdown').innerHTML=bd;
  document.getElementById('btnNextShop').textContent=t('btn_next_shop');
  document.getElementById('resultModal').classList.remove('hidden');

  // Animate life change
  const prev=+(G.life-result.netGain).toFixed(2);
  setTimeout(()=>animateLifeChange(prev, G.life, 1000), 200);
}

function closeResultAndShop() {
...
function renderBetBar() {
  if (!G) return;
  document.getElementById('betAmountDisplay').textContent=G.betAmount.toFixed(1)+'♥';
  ...
  const fill=document.getElementById('betBarFill');
...
function renderHandResult() {
  ...
  document.getElementById('betInfo').textContent=`×${G.betAmount.toFixed(1)} bet × ×${result.mult.toFixed(2)} mult`;
  ...
  if (sb?.suit) {
    ...
    txt.textContent=`+${(sb.bonus*100).toFixed(1)}%`;
...
function renderButtons() {
  ...
  document.getElementById('phaseInfo').textContent=inPlay
    ?t('phase_betting',G.round,G.revealedCount)
    :t('phase_processing');

  // Bet row visibility
  document.getElementById('betRow').style.display=inPlay?'flex':'none';
}
